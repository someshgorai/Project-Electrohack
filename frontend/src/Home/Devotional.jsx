...
import { Link } from 'react-router-dom';
  ...
  function Devotional(){
    ...
    const DevotionalBlogs = blogs?.filter((blogs)=> blogs.category=== "Devotional");
...
<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
     ...
     DevotionalBlogs.map((blog,index) => {
        ...
        <Link to={`/blog/${blog.id}`} key={index} className='  relative rounded-lg overflow-hidden shadow-md   transform hover:scale-105 transition-transform duration-300  ' >
